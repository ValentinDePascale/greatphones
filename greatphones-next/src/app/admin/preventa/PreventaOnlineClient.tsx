          ...
          {cargandoActivas ? (
            ...
            >
              {preventasActivas.map(dispositivo => (
                ...
                >
                  <div style={{ display: 'flex', gap: 12, padding: 14, borderBottom: '1px solid #E6E7F0', alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#8892A6' }}>
                        {dispositivo.variantes.length} variante{dispositivo.variantes.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                    <button
                      onClick={() => handleEliminarDispositivoActivo(dispositivo.modelGroup)}
                      title="Eliminar dispositivo completo"
                      style={{
                        width: 30,
                        height: 30,
                        flexShrink: 0,
                  ...
                  <div style={{ padding: 8 }}>
                    {dispositivo.variantes.map(variante => {
                      const editando = editandoVarianteId === variante.id
                        ...
                        return (
                          ...
                          >
                            <div style={{ fontSize: 12, fontWeight: 700, color: '#181B2E', marginBottom: 6 }}>
                              {variante.color}{variante.storage ? ` · ${variante.storage}` : ''}
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button
                                onClick={async () => {
                                  await handleGuardarVariante(variante.id)
                              ...
                              <button
                                onClick={() => setEditandoVarianteId(null)}
                      ...
                      // Fila compacta: texto + botones editar/eliminar
                      return (
                        <div
                          key={variante.id}
                          style={{
                        ...
                        >
                          <div style={{ flex: 1, minWidth: 0, fontSize: 12 }}>
                            <span style={{ fontWeight: 700, color: '#181B2E' }}>{variante.color}</span>
                            {variante.storage && <span style={{ color: '#6B7280' }}> · {variante.storage}</span>}
                            <span style={{ color: '#6B7280' }}> · {fmt(variante.price)}</span>
                          </div>
                          <button
                            onClick={() => setEditandoVarianteId(variante.id)}
                            title="Editar"
                            style={{
                              width: 26,
                              height: 26,
