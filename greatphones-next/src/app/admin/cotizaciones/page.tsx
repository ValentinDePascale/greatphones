...
export default async function AdminCotizaciones() {
  ...
  const kpis = [
    ...
    { label: 'Tasa de aceptación', value: acceptance, color: '#1a1a1a' },
  ]

  return (
    <div className="page-xl" style={{ background: '#FDF8F3', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
        <a href="/admin" style={{ color: '#FF6B2C', textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>← Admin</a>
      </div>
      <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700, color: '#1a1a1a', marginBottom: 4 }}>Dashboard de Cotizaciones</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E4DDD4', padding: '1.25rem' }}>
          ...
          {byDevice.length === 0 ? <p style={{ fontSize: 13, color: '#9A9186' }}>Sin datos</p> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {byDevice.map(b => (
                <div key={b.device} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 120, fontSize: 12, fontWeight: 600, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.device}</div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E4DDD4', padding: '1.25rem' }}>
          ...
          {byStatus.length === 0 ? <p style={{ fontSize: 13, color: '#9A9186' }}>Sin datos</p> : (
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E4DDD4', padding: '1.25rem' }}>
        ...
        {recentQuotes.length === 0 ? <p style={{ fontSize: 13, color: '#9A9186' }}>Sin cotizaciones</p> : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead><tr style={{ borderBottom: '2px solid #E4DDD4', background: '#FDF8F3' }}>
            ...
            <tbody>
              {recentQuotes.map(q => (
                <tr key={q.code} style={{ borderBottom: '1px solid #F0EBE3' }}>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: q.status === 'ACCEPTED' ? '#2D5A27' : '#1a1a1a' }}>{fmt(q.finalPrice)}</td>
                  <td style={{ padding: '10px 12px', textAlign: 'center' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 700, background: q.status === 'ACCEPTED' ? 'rgba(45,90,39,.1)' : q.status === 'PENDING' ? 'rgba(255,107,44,.1)' : q.status === 'REJECTED' ? 'rgba(192,57,43,.1)' : 'rgba(107,98,89,.1)', color: q.status === 'ACCEPTED' ? '#2D5A27' : q.status === 'PENDING' ? '#FF6B2C' : q.status === 'REJECTED' ? '#c0392b' : '#6B6259' }}>{statusMap[q.status] || q.status}</span>
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', fontSize: 11, color: '#9A9186' }}>{new Date(q.createdAt).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit' })}</td>
                </tr>
              ))}
            </tbody>
          </table>
