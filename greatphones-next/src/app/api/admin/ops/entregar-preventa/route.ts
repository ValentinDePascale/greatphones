...
export async function POST(request: Request) {
  try {
    ...
    if (totalAcc > 0 && !d.inventoryItemId) return NextResponse.json({ error: 'Para entregar con acceso esos, seleccioná el equipo' }, { status: 400 })

    // Cobro del saldo
    const saldo = pre.price || 0
    const cobradoAhoraPesos = d.efectivo + d.transferencia + d.cuotas + Math.round((d.usd || 0) * 1000)

    // Marcar el equipo como vendido si se eligió
    if (d.inventoryItemId) {
      await prisma.inventoryItem.update({ where: { id: d.inventoryItemId }, data: { status: 'SOLD', soldAt: new Date(), soldById: admin.id } }).catch(e => console.error('[Ops Entrega] stock:', e))
    }

    const numero = 'PRE-ENTREGA-' + Date.now().toString().slice(-7)

    // Asientos del saldo cobrado
    const medios = [
    ...
    await prisma.preOrder.update({
      where: { id: pre.id },
      data: { status: 'DELIVERED', deliveredAt: new Date(), notes: (pre.notes || '') + ` | Entregado ${new Date().toISOString()}` },
    })
