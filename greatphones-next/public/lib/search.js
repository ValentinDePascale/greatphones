...
function handleSearch(q){
  ...
  dd.innerHTML=m.map(function(item){
    ...
    return '<div class="srdd-item" onclick="'+clickAction+';document.getElementById(\'searchDD\').classList.remove(\'open\')">'+escapeHtml(item.ico||'\u{1F4E6}')+' '+escapeHtml(item.name)+tag+'</div>';
  }).join('');
  dd.classList.add('open');
}
function doSearch(){
